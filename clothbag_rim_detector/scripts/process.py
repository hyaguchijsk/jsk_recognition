...
def process():
  ...
  try:
    ...
    bag_area_mask = (erode_and_dilate_filter(bag_area_mask.astype("uint8"), 7)).astype("bool")
    ...
    if hand=="rhand":
      ...
      # hand_xyz_correction = np.array([0.,0.,0])
    elif hand=="lhand":
    ...
    plane_distance = calc_dist_point_to_plane(camera.xyz_world, plane_param, True)
    ...
    # cv2.fillConvexPoly(in_plane_color, in_plane_contour[0], (0,255,0), lineType=4)
    ...
    plane_distance_color[(plane_distance<-d_margin)*(in_plane_mask)] = (255,0,255)
    ...
    try:
      ...
      # cv2.line(color_edit, tuple(mouse_point_left), tuple(mouse_point_right), (0,255,255), 3)
    ...
    # xy = depth.nonzero()
    # x_min, y_min, x_max, y_max = xy[1].min(), xy[0].min(), xy[1].max(), xy[0].max()
    # # depth[depth==0] = PLANE_HEIGHT*1000
    # canny = cv2.Canny(depth2gray(depth),100,255)
    # show_image(canny, "canny", False)

    # hand
    # color_edit = draw_points(color_edit, [camera.rhand_point, camera.lhand_point], (0,255,0))
    ...
    # show_image(depth2gray(depth_world_edit), "depth_world_edit", SET_ON_MOUSE, DRAW_GRID)
    # show_image(depth2gray(depth_world_edit, 750, 850, focus=False), "depth_cut", SET_ON_MOUSE, DRAW_GRID)
    # show_image(depth2gray(depth_plane_model, 710, 750, focus=False), "depth_plane_model", SET_ON_MOUSE, DRAW_GRID)
