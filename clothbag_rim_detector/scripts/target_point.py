def calc_target_point_xyz(task_stage):
  ...
  global PICK_BAG_TARGET, CHANGE_HAND_TARGET, RAISE_HAND_TARGET, PUT_OBJECT_TARGET
  ...
  if task_stage == "pick_bag":
    ...
    dist_list = calc_dist_point_to_point(camera.xyz_world[lower_rim], p)
    if len(dist_list) == 0:
      print "target_point/pick_bag: lower_rim not found "
      return
    ...
    publish_tf_theta(target_xyz, BAG_DIRECTION_THETA, "pick_bag_position")
    publish_tf_theta(pre_target_xyz, BAG_DIRECTION_THETA, "pre_pick_bag_position")
    publish_tf_theta(after_target_xyz, BAG_DIRECTION_THETA, "after_pick_bag_position")
    return camera.xyz_world2point(target_xyz)

  elif task_stage == "change_hand":
    p = hand_xyz_world - CHANGE_HAND_TARGET/1000.0 * v2
    dist_list = calc_dist_point_to_point(camera.xyz_world[upper_rim], p)
    argmin = (dist_list**2).argmin()
    ...
    publish_tf_theta(target_xyz, BAG_DIRECTION_THETA, "change_hand_position")
    publish_tf_theta(pre_target_xyz, BAG_DIRECTION_THETA, "pre_change_hand_position")
    ...
    # print target_xyz, hand_xyz_world, np.sqrt(((target_xyz-hand_xyz_world)**2).sum())

    publish_tf_theta(target_xyz, BAG_DIRECTION_THETA, "raise_hand_position")
    publish_tf_theta(reverse_target_xyz, BAG_DIRECTION_THETA, "raise_hand_reverse_position")
    ...
    publish_tf_theta(pre_target_xyz, BAG_DIRECTION_THETA+np.pi/2, "pre_put_object_position")
    publish_tf_theta(target_xyz, BAG_DIRECTION_THETA+np.pi/2, "put_object_position")
